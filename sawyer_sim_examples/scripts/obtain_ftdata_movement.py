...
def move_to_pos(robot,pos_command,step):

        rospy.loginfo("Moving to %s, with a step of %s", pos_command,step)

        if pos_command == "None":
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[0.0,0.0,0.0])
        elif pos_command == "X+":
            robot.move_to_cartesian_relative(position=[step,0.0,0.0],orientation=[0.0,0.0,0.0])
        elif pos_command == "X-":
            robot.move_to_cartesian_relative(position=[-step,0.0,0.0],orientation=[0.0,0.0,0.0])
        elif pos_command == "Y+":
            robot.move_to_cartesian_relative(position=[0.0,step,0.0],orientation=[0.0,0.0,0.0])
        elif pos_command == "Y-":
            robot.move_to_cartesian_relative(position=[0.0,-step,0.0],orientation=[0.0,0.0,0.0])
        elif pos_command == "Z+":
            robot.move_to_cartesian_relative(position=[0.0,0.0,step],orientation=[0.0,0.0,0.0])
        elif pos_command == "Z-":
            robot.move_to_cartesian_relative(position=[0.0,0.0,-step],orientation=[0.0,0.0,0.0])
        elif pos_command == "RotX+":
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[step,0.0,0.0])
        elif pos_command == "RotX-":
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[-step,0.0,0.0])
        elif pos_command == "RotY+":
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[0.0,step,0.0])
        elif pos_command == "RotY-":
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[0.0,-step,0.0])
        elif pos_command == "RotZ+":
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[0.0,0.0,step])
        elif pos_command == "RotZ-":
            robot.move_to_cartesian_relative(position=[0.0,0.0,0.0],orientation=[0.0,0.0,-step])
        elif pos_command == "X+Y+":
            robot.move_to_cartesian_relative(position=[step,step,0.0],orientation=[0.0,0.0,0.0])
        elif pos_command == "X+Y-":
            robot.move_to_cartesian_relative(position=[step,-step,0.0],orientation=[0.0,0.0,0.0])
        elif pos_command == "X-Y+":
            robot.move_to_cartesian_relative(position=[-step,step,0.0],orientation=[0.0,0.0,0.0])
...
def adquisition_cycle(robot,pos_command,step, test_time=1.0, category=[]):
    ...
    try:
        ...
        move_to_pos(robot,pos_command,step)
...
if __name__ == '__main__':
    ...
    try:
        ...
        for movement in test_movements:
            ...
            adquisition_cycle(robot=sawyer, pos_command=direction, step=0.1, test_time=1.0, category=category)
